"""
SCUNet Real-PSNR Model Wrapper for Blind Image Denoising.

SCUNet (Practical Swin-Conv UNet for Blind Image Denoising) accepts an RGB image
and outputs a clean, denoised image using the Real-PSNR checkpoint.
"""
...
def trunc_normal_(tensor, mean=0., std=1., a=-2., b=2.):
    with torch.no_grad():
        return tensor.normal_(mean, std).clamp_(a * std, b * std)
...
class SCUNetModel(BaseRestorationModel):
    """
    SCUNet Denoising Wrapper.
    Loads PyTorch Real-PSNR checkpoint and performs blind image denoising.
    Exposes the standard BaseRestorationModel interface.
    """

    def __init__(self, config: dict[str, Any] | None = None, device: str | None = None):
        super().__init__(config, device)
        self.model: SCUNetArch | None = None

    def load(self) -> None:
        self.model = SCUNetArch(in_nc=3, config=[4, 4, 4, 4, 4, 4, 4]).to(self.device)
        self.model.eval()

        repo_id = self.config.get("weights_repo", "Ansh205/image-restoration-models")
        weights_path = self.config.get("weights_path", "scunet/scunet_color_real_psnr.pth")
